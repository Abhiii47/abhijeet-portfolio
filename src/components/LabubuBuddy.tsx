...
export default function LabubuBuddy() {
  ...
  useEffect(() => {
    setPos({ x: window.innerWidth - 110, y: window.innerHeight - 160 });
    setIsInit(true);
  }, []);
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      // Arm angle points toward cursor
      setArmAngle(Math.atan2(dy, dx) * (180 / Math.PI));
    };
      ...
      >
        ...
        <g
          style={{
            ...
            transform: `rotate(${isWaving ? Math.sin(Date.now() / 200) * 30 - 30 : -10}deg)`,
