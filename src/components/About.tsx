...
export default function About() {
  ...
  useGSAP(() => {
    // All fromTo — end state always guaranteed
    ...
    gsap.fromTo(colLeftRef.current,
      ...
      { x: 0, opacity: 1, duration: 0.75, ease: "power3.out",
        scrollTrigger: { trigger: colLeftRef.current, start: "top 82%", once: true } }
    );
    gsap.fromTo(colRightRef.current,
      { y: 28, opacity: 0 },
      { y: 0, opacity: 1, duration: 0.75, ease: "power3.out",
        scrollTrigger: { trigger: colRightRef.current, start: "top 82%", once: true } }
    );
  }, { scope: ref });
    ...
    >
      ...
      <div style={{ maxWidth: 1140, margin: "0 auto", position: "relative", zIndex: 1 }}>
        ...
        <div className="about-2col" style={{
          ...
          <div ref={colRightRef} style={{ display: "flex", flexDirection: "column", gap: 20, paddingTop: 4 }}>
            {[
        ...
        >
          ...
          <BentoCell delay={0.18} span={3}>
            ...
            <div style={{ display:"flex", flexWrap:"wrap", gap:7 }}>
              {STACK.map((t, i) => (
                <span key={t} className={i < 5 ? "tag-hot" : "tag-normal"} style={{ cursor:"default" }}>{t}</span>
              ))}
            </div>
          </BentoCell>
        </div>
