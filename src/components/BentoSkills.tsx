...
const RINGS: {
  ...
  ring: 1 | 2 | 3; angle: number; desc: string; tags: string[];
}[] = [
...
const RING_RADII    = { 1: 130, 2: 215, 3: 300 };
...
function Core() {
  return (
    <div style={{
      ...
      background: `radial-gradient(circle, rgba(196,64,10,0.16) 0%, rgba(196,64,10,0.04) 60%, transparent 100%)`,
      border: `1.5px solid rgba(196,64,10,0.35)`,
      display: "flex", alignItems: "center", justifyContent: "center",
      boxShadow: `0 0 32px rgba(196,64,10,0.12)`,
  ...
  return (
    ...
    >
      <circle
        ...
        stroke={`rgba(26,18,8,${dashed ? 0.08 : 0.10})`}
...
function SkillNode({ skill, radius, isActive, onClick, isMobile }: {
  skill: SkillNode; radius: number; isActive: boolean; onClick: () => void; isMobile: boolean;
}) {
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: "50%",
        background: isActive ? `rgba(196,64,10,0.12)` : "rgba(255,252,246,0.95)",
        border: isActive ? `1.5px solid ${ACCENT}` : `1px solid rgba(26,18,8,0.12)`,
        boxShadow: isActive
          ? `0 0 18px rgba(196,64,10,0.28), 0 4px 12px rgba(0,0,0,0.06)`
          : `0 2px 8px rgba(0,0,0,0.06)`,
        display: "flex", alignItems: "center", justifyContent: "center",
        cursor: "pointer",
        transition: "all 0.25s cubic-bezier(0.16,1,0.3,1)",
        zIndex: isActive ? 20 : 5, padding: 0,
      }}
      onMouseEnter={e => {
        if (!isActive) {
          (e.currentTarget as HTMLElement).style.background = `rgba(196,64,10,0.07)`;
          (e.currentTarget as HTMLElement).style.borderColor = `rgba(196,64,10,0.4)`;
          (e.currentTarget as HTMLElement).style.transform = `translate(calc(-50% + ${x}px), calc(-50% + ${y}px)) scale(1.15)`;
        }
      }}
      onMouseLeave={e => {
        if (!isActive) {
          (e.currentTarget as HTMLElement).style.background = "rgba(255,252,246,0.95)";
          (e.currentTarget as HTMLElement).style.borderColor = "rgba(26,18,8,0.12)";
          (e.currentTarget as HTMLElement).style.transform = `translate(calc(-50% + ${x}px), calc(-50% + ${y}px)) scale(1)`;
        }
      }}
    >
      <BrandIcon icon={skill.icon} color={isActive ? "C4400A" : skill.color.replace("#","")} size={isMobile ? 16 : 20} />
    </button>
  );
}
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "12px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <span style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 700, fontSize: "1.1rem", color: INK }}>{skill.label}</span>
        </div>
        <button
          onClick={onClose} aria-label="Close"
          style={{ color: "rgba(26,18,8,0.35)", background: "none", border: "none", cursor: "pointer", lineHeight: 1 }}
          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = INK; }}
          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = "rgba(26,18,8,0.35)"; }}
        >&#x2715;</button>
      </div>
      <p style={{ fontFamily: "system-ui, sans-serif", fontSize: "12px", color: "rgba(26,18,8,0.6)", lineHeight: 1.65, marginBottom: "12px" }}>{skill.desc}</p>
      ...
      <div style={{
        ...
        boxShadow: `0 0 12px rgba(196,64,10,0.5)`,
        pointerEvents: "none",
      }} />
    </div>
  );
...
export default function BentoSkills() {
  ...
  return (
    <section
      id="skills"
      ref={sectionRef}
      style={{ background: CREAM, color: INK, position: "relative", overflow: "hidden" }}
    ...
    >
      ...
      <span aria-hidden style={{
        ...
        color: "transparent", WebkitTextStroke: "1px rgba(196,64,10,0.06)",
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-16">
          ...
          <p className="skills-head mt-3" style={{ fontSize: "clamp(0.85rem,1vw,1rem)", color: "rgba(26,18,8,0.48)", maxWidth: "48ch" }}>
            Click any node to explore. Hover to preview. Built across cloud, ML, and full-stack.
          </p>
        </div>

        <div style={{ display: "flex", justifyContent: "center" }}>
          <div ref={orbitRef} style={{ position: "relative", width: orbitSize, height: orbitSize, flexShrink: 0 }}>
            <OrbitRing r={radii[1]} />
            ...
            {RINGS.map(skill => (
              <div key={skill.label} className="skill-node-wrap" style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", pointerEvents: "none" }}>
                <div style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", pointerEvents: "auto" }}>
                  <SkillNode
                    skill={skill} radius={radii[skill.ring]}
            ...
            {activeSkill && <DetailPanel skill={activeSkill} onClose={() => setActive(null)} />}
          </div>
        </div>
        ...
        <div className="flex items-center justify-center gap-8 mt-12 flex-wrap">
          {([1,2,3] as const).map(r => (
            <div key={r} style={{ display: "flex", alignItems: "center", gap: "6px" }}>
              <div style={{ width: 24, height: 1, background: r === 2 ? `repeating-linear-gradient(90deg,rgba(26,18,8,0.22) 0,rgba(26,18,8,0.22) 4px,transparent 4px,transparent 12px)` : "rgba(26,18,8,0.2)" }} />
              <span style={{ fontFamily: "monospace", fontSize: "9px", letterSpacing: "0.2em", textTransform: "uppercase", color: "rgba(26,18,8,0.38)" }}>
      ...
      <style>{`
        @keyframes corePulse {
          0%,100% { box-shadow: 0 0 32px rgba(196,64,10,0.10); }
          50%      { box-shadow: 0 0 52px rgba(196,64,10,0.22); }
        }
        @keyframes panelIn {
