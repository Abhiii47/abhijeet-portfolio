...
const SKILLS: Skill[] = [
  ...
  { name: "Next.js",     icon: "nextjs",         color: "#555555", level: "expert",  category: "Frontend",  stat: "60% less reports",  desc: "Full-stack apps, App Router, ISR. 60% drop in manual reporting at Ecovis.", tags: ["SSR","ISR","App Router"] },
...
/* ── Brand icon via simpleicons CDN ── */
...
/* ── Large card for Expert-level skills ── */
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hovered
          ? `0 20px 48px ${skill.color}18, 4px 4px 0 ${skill.color}14`
    ...
    >
      ...
      <div aria-hidden style={{
        ...
        background: `radial-gradient(ellipse 80% 60% at 100% 0%, ${skill.color}10 0%, transparent 70%)`,
      ...
      <div style={{
        ...
        background: `${ACCENT}12`, border: `1px solid ${ACCENT}35`, color: ACCENT, fontWeight: 700,
      }}>
      ...
      <div style={{
        ...
        background: `${skill.color}12`, border: `1.5px solid ${skill.color}28`,
        display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 14,
      }}>
        <BrandIcon icon={skill.icon} color={skill.color} size={24} />
      </div>

      <p style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "clamp(1.05rem,1.4vw,1.2rem)", color: INK, letterSpacing: "-0.02em" }}>
        {skill.name}
      </p>
      {skill.stat && (
        <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.58rem", letterSpacing: "0.22em", textTransform: "uppercase", color: skill.color, marginTop: 4, fontWeight: 700 }}>
          {skill.stat}
        </p>
      )}
      ...
      {/* Progress bar */}
      <div style={{ marginTop: 16, height: 2, background: "rgba(14,10,4,0.07)", borderRadius: 99, overflow: "hidden" }}>
        <div style={{
          height: "100%", width: `${LEVEL_BAR[skill.level]}%`, borderRadius: 99,
...
/* ── Small compact card for Proficient / Working skills ── */
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hovered ? `0 10px 28px ${skill.color}10` : "none",
        cursor: "default",
      }}
    >
      <div aria-hidden style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
        <div style={{
          ...
          background: `${skill.color}10`, border: `1px solid ${skill.color}22`,
        ...
        <div style={{ minWidth: 0 }}>
          ...
          {skill.stat && (
            <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.48rem", letterSpacing: "0.2em", textTransform: "uppercase", color: skill.color, marginTop: 2, fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
              {skill.stat}
            </p>
          )}
        </div>
      ...
      <div style={{ height: 2, background: "rgba(14,10,4,0.06)", borderRadius: 99, overflow: "hidden" }}>
        <div style={{
          height: "100%", width: `${LEVEL_BAR[skill.level]}%`, borderRadius: 99,
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 10 }}>
        {skill.tags.slice(0, 3).map(t => (
          <span key={t} className="tag-normal" style={{ fontSize: "0.48rem" }}>{t}</span>
        ))}
      </div>
    </div>
  );
...
export default function Skills() {
  ...
  const totalExperts = SKILLS.filter(s => s.level === "expert").length;
  const totalStrong  = SKILLS.filter(s => s.level === "strong").length;
  ...
  return (
    ...
    >
      ...
      <style>{`
        ...
        .sk-reveal {
          opacity: 0;
          animation: skReveal 0.5s cubic-bezier(0.16,1,0.3,1) forwards;
        }
        .sk-cat-btn {
          position: relative;
        ...
        .sk-cat-btn:hover { color: rgba(14,10,4,0.75); border-color: rgba(14,10,4,0.22); }
        .sk-cat-btn.active {
          background: rgba(196,64,10,0.07);
          border-color: rgba(196,64,10,0.35);
          color: #C4400A;
        }
        .sk-cat-btn.active::after {
          content: '';
          position: absolute;
      ...
      <div style={{ maxWidth: 1160, margin: "0 auto", position: "relative" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 16, marginBottom: "clamp(28px,4vw,44px)" }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <RMSticker text={`${totalExperts} expert`} accent rotate={-1.5} />
            <RMSticker text={`${totalStrong} proficient`} rotate={1.5} />
          </div>
        </div>
        ...
        <div style={{
          ...
          {[
            { label: "Expert",     value: totalExperts, color: ACCENT },
            { label: "Proficient", value: totalStrong,  color: "rgba(14,10,4,0.55)" },
            { label: "Total",      value: SKILLS.length, color: "rgba(14,10,4,0.30)" },
            ...
            <div key={label} style={{
              ...
              <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.58rem", letterSpacing: "0.22em", textTransform: "uppercase", color: "rgba(14,10,4,0.35)", lineHeight: 1.4 }}>
                {label}<br />Skills
              </span>
            </div>
          ))}
        ...
        {/* Category filters */}
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: "clamp(16px,2.5vw,28px)" }}>
          {CATEGORIES.map(cat => (
            <button
              key={cat}
        ...
        {/* Expert / Featured grid */}
        {experts.length > 0 && (
          <>
            <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.56rem", letterSpacing: "0.32em", textTransform: "uppercase", color: `${ACCENT}88`, marginBottom: 12 }}>
              ★ Expert level
            </p>
            <div style={{
              display: "grid",
        ...
        {/* Proficient / Working grid */}
        {rest.length > 0 && (
          <>
            {experts.length > 0 && (
              <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.56rem", letterSpacing: "0.32em", textTransform: "uppercase", color: "rgba(14,10,4,0.28)", marginBottom: 12 }}>
            ...
            <div style={{
              ...
              {rest.map((skill, i) => (
                <CompactCard key={skill.name} skill={skill} index={experts.length + i} />
              ))}
            </div>
          </>
        )}
        ...
        <div style={{ display: "flex", gap: 20, marginTop: "clamp(28px,3.5vw,40px)", flexWrap: "wrap", alignItems: "center" }}>
          {([
            { label: "Expert",    color: ACCENT,                   pct: 95 },
            { label: "Proficient", color: "rgba(14,10,4,0.55)",    pct: 72 },
            { label: "Working",   color: "rgba(14,10,4,0.28)",     pct: 40 },
          ]).map(({ label, color, pct }) => (
            <div key={label} style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 32, height: 2, background: "rgba(14,10,4,0.07)", borderRadius: 99, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${pct}%`, background: color, borderRadius: 99 }} />
              </div>
              <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.54rem", letterSpacing: "0.22em", textTransform: "uppercase", color: "rgba(14,10,4,0.38)" }}>{label}</span>
            </div>
          ))}
        </div>
      </div>
