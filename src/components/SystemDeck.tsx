...
export default function SystemDeck() {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    gsap.to(deckRef.current, { rotateY: x * 14, rotateX: -y * 10, duration: 0.5, ease: "power2.out" });
  };

  const handleMouseLeave = () => {
    gsap.to(deckRef.current, { rotateY: 0, rotateX: 0, duration: 1.2, ease: "elastic.out(1, 0.4)" });
  };
        ...
        >
          ...
          >
            <div
              ...
              style={{ background: "#0a0a0a", transform: "translateZ(-14px)", boxShadow: "0 20px 40px rgba(0,0,0,0.6)" }}
            />
            ...
            {modules.map((mod) => {
              ...
              return (
                <div
                  key={mod.id}
                  className="module-key relative w-16 h-16 cursor-pointer"
                  style={{
                    transformStyle: "preserve-3d",
                ...
                >
                  <div
                    ...
                    style={{
                      background: isActive ? `linear-gradient(145deg, ${mod.color}22, #1a1a1a)` : "#1c1c1c",
                      border: isActive || isHovered ? `1px solid ${mod.color}60` : "1px solid rgba(255,255,255,0.06)",
                      boxShadow: isActive
                        ? `0 0 20px ${mod.color}40, inset 0 1px 0 ${mod.color}30`
                        : isHovered ? `0 0 10px ${mod.color}25`
                  ...
                  >
                    <div
                      ...
                      style={{ background: isActive ? `${mod.color}12` : "#181818", transform: "translateZ(2px)" }}
                    >
                      <div
                        ...
                        style={{
                          ...
                          boxShadow: isActive || isHovered ? `0 0 8px ${mod.color}, 0 0 16px ${mod.color}60` : "none",
                    ...
                    <div className="absolute inset-x-0 bottom-0 h-[6px] rounded-b-[5px]" style={{ background: "rgba(0,0,0,0.5)" }} />
                  </div>
                </div>
              );
            })}
          ...
          >
            <div className="flex items-center gap-2 px-4 py-3 border-b" style={{ borderColor: `${accentColor}20` }}>
              <div className="w-2.5 h-2.5 rounded-full bg-red-500/60" />
              <div className="w-2.5 h-2.5 rounded-full bg-yellow-500/60" />
              <div className="w-2.5 h-2.5 rounded-full bg-green-500/60" />
              <span className="ml-auto text-[9px] tracking-widest uppercase" style={{ color: accentColor, opacity: 0.7 }}>sys.inspect</span>
            </div>

            <div className="p-5 min-h-[200px] space-y-4">
              {!activeModule ? (
                <>
                  <p className="text-[11px] animate-pulse" style={{ color: accentColor }}>&gt; AWAITING MODULE SELECTION...</p>
                  <p className="text-[11px] text-gray-700">Click any key to inspect parameters.</p>
                  <div className="mt-6 space-y-1">
                    {["active", "standby", "warning"].map(s => (
                      <div key={s} className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 rounded-full" style={{ background: s === "active" ? "#84cc16" : s === "standby" ? "#64748b" : "#f97316" }} />
                ...
                <>
                  <div className="flex items-center justify-between pb-3 border-b" style={{ borderColor: `${activeModule.color}25` }}>
                    <div>
                      <p className="text-base font-bold tracking-wider" style={{ color: activeModule.color }}>{activeModule.label}</p>
                      <p className="text-[9px] text-gray-600 mt-0.5 uppercase tracking-widest">{activeModule.id} · module</p>
                    </div>
                    <span
                      className="text-[9px] px-2 py-1 rounded uppercase tracking-widest"
                      style={{ background: `${activeModule.color}15`, color: activeModule.color, border: `1px solid ${activeModule.color}30` }}
                    >{activeModule.status}</span>
                  </div>
                  <p className="text-[11px] text-gray-400 leading-relaxed">{activeModule.desc}</p>
                  <div className="grid grid-cols-3 gap-2 pt-2 text-[9px]" style={{ borderTop: `1px solid ${activeModule.color}15` }}>
